...
import dev.projects.math.jnummath.jlinalgebra.contexts.VectorContext;
...
public class DenseVectorV2 implements IVector {
    ...
    public static IVector getInstance(int size, int switchThreshold)   {
    ...
    public static IVector getInstance(int size)   {
        return getInstance(size, MAX_SWITCH_THRESHOLD);
    }

    public static IVector getInstance(double[] data, int switchThreshold)   {
    ...
    public static IVector getInstance(double[] data)   {
        return getInstance(data, MAX_SWITCH_THRESHOLD);
    }

    public static IVector getInstance(int size, int switchThreshold, double value)   {
    ...
    public static IVector getRandomizedVector(int length, double scalFactor)   {
    ...
    private DenseVectorV2(IVector vector)   {
    ...
    @Override
    public IVector multiplyByCoefficient(double coeff)   {
        for (IDataBlock block : getData().values()) {
    ...
    @Override
    public IVector shiftByCoefficient(double coeff)   {
        for (IDataBlock block : getData().values()) {
    ...
    @Override
    public IMatrix multiplyByVector(IVector vector)   {
        if ((vector == null)
    ...
    @Override
    public IVector setData(Map<RegionDataBlockKey, IDataBlock> data)   {
    ...
    @Override
    public IVector add(double coeff, IVector vector)   {
        return null;
    }
    ...
    @Override
    public IVector substract(double coeff, IVector vector)   {
        return null;
    }

    @Override
    public double getInnerProduct(IVector vector)   {
        if (vector == null) throw new LoggableException("Unacceptable vector");
    ...
    @Override
    public IVector convertToColumnVector()   {
    ...
    @Override
    public IVector convertToRowVector()   {
    ...
    @Override
    public IVector extendByVector(IVector vector)   {
        return null;
    }

    @Override
    public IVector extendByDouble(double value)   {
        return extendByArray(new double[]{value});
    }

    @Override
    public IVector extendByArray(double[] array)   {
        return null;
    }

    @Override
    public IVector[] splitMiddleToLeftAtIndex(int index)   {
        return splitAtIndex(index, true);
    }

    @Override
    public IVector[] splitMiddleToRigthAtIndex(int index)   {
        return splitAtIndex(index, false);
    }

    @Override
    public IVector[] splitAtIndex(int index, boolean middleToLeft)   {
        return new IVector[0];
    }

    @Override
    public IVector setValue(int index, double value)   {
